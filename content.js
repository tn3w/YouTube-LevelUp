(() => {
    ...
    const backgroundPlayback = {
        ...
        applySettings: (settings) => {
            backgroundPlayback.setAttribute('data-yt-bg-enabled', settings.enabled ? '1' : '0');
            backgroundPlayback.setAttribute(
                'data-yt-bg-block-events',
                settings.blockVisibilityEvents ? '1' : '0'
            );
            backgroundPlayback.setAttribute('data-yt-bg-keepalive', settings.keepAlive ? '1' : '0');
            backgroundPlayback.setAttribute('data-yt-bg-method', settings.keepAliveMethod);
        },

        loadSettings: async () => {
            const api = backgroundPlayback.getStorageApi();
            if (!api?.storage?.local) return backgroundPlayback.DEFAULTS;

            try {
                const keys = Object.keys(backgroundPlayback.DEFAULTS);
                ...
                return backgroundPlayback.DEFAULTS;
            }
        },
...
`;
        ...
        init: async () => {
            const settings = await backgroundPlayback.loadSettings();
            backgroundPlayback.applySettings(settings);

            backgroundPlayback.setAttribute('data-yt-bg-loaded', '1');
            backgroundPlayback.setAttribute('data-yt-bg-loaded-at', String(Date.now()));

            backgroundPlayback.injectMainWorldScript();
            ...
            if (api?.storage?.onChanged) {
                api.storage.onChanged.addListener((changes, area) => {
                    ...
                    backgroundPlayback.applySettings(updated);
                });
            }
        },
    };
